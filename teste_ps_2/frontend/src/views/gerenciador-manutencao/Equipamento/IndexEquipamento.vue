<template>
    ...
    <v-row>
      ...
      <v-col cols="4" class="d-flex align-start">
        <v-btn class="custom-width" color="primary" variant="flat" dark @click="searchEquipamento" name="SearchButton">Buscar</v-btn>
      </v-col>
      <v-col cols="2" class="d-flex justify-end">
        <v-btn class="custom-width-2" color="primary" variant="flat" dark @click="addEquipamento" name="CreateButton">Cadastrar Equipamento</v-btn>
      </v-col>
    </v-row>
    <v-row>
      <v-col cols="12">
        <v-data-table class="border rounded-md" :headers="headers" :items="filteredEquipamento">
          <template v-slot:top>
            <v-row>
...
import EquipamentoRequires from '../../../services/requires/EquipamentoRequires';

const { list, remove, update, post } = EquipamentoRequires();
const page = ref({ title: 'Equipamento ' });
const breadcrumbs = ref([
  { text: 'Equipamento', disabled: true, href: '#' },
...
const headers = ref([
  ...
  { title: 'HorimetroOuOdometro', sortable: false, key: 'horimetroOuOdometro' },

  { title: 'Ações', key: 'actions' }
]);
const Equipamento = ref([]);
const filteredEquipamento = ref([]);
...
const getPosts = async () => {
  try {
    ...
    filteredEquipamento.value = data;
  } catch (error) {
...
function addEquipamento() {
  router.push('/Equipamento/formEquipamento');
}
...
function goToDetail(id: any) {
  router.push({ path: `/Equipamento/detailsEquipamento/${id}` });
}
...
const searchEquipamento = () => {
  filterEquipamento();
};

const filterEquipamento = () => {
  if (!search.value) {
    filteredEquipamento.value = Equipamento.value;
  } else {
    const searchQuery = search.value.toLowerCase();
    filteredEquipamento.value = Equipamento.value.filter(item =>
