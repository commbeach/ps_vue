<template>
    ...
    <v-row>
      <v-col cols="6">
        <v-text-field
          v-model="search"
          label="Buscar OrdemServico"
          clearable
        ></v-text-field>
      </v-col>
      <v-col cols="4" class="d-flex align-start">
        <v-btn class="custom-width" color="primary" variant="flat" dark @click="searchOrdemServico" name="SearchButton">Buscar</v-btn>
      </v-col>
      <v-col cols="2" class="d-flex justify-end">
        <v-btn class="custom-width-2" color="primary" variant="flat" dark @click="addOrdemServico" name="CreateButton">Cadastrar OrdemServico</v-btn>
      </v-col>
    </v-row>
    <v-row>
      <v-col cols="12">
        <v-data-table class="border rounded-md" :headers="headers" :items="filteredOrdemServico">
          <template v-slot:top>
            <v-row>
...
import OrdemServicoRequires from '../../../services/requires/OrdemServicoRequires';

const { list, remove, update, post } = OrdemServicoRequires();
const page = ref({ title: 'OrdemServico ' });
const breadcrumbs = ref([
  { text: 'OrdemServico', disabled: true, href: '#' },
...
const headers = ref([
  ...
  { title: 'DataAbertura', sortable: false, key: 'DataAbertura' },
  { title: 'DataFinalizacao', sortable: false, key: 'DataFinalizacao' },

  { title: 'Ações', key: 'actions' }
]);
const OrdemServico = ref([]);
const filteredOrdemServico = ref([]);
...
const getPosts = async () => {
  try {
    ...
    OrdemServico.value = data.value;
    filteredOrdemServico.value = data.value;
  } catch (error) {
...
function addOrdemServico() {
  router.push('/OrdemServico/formOrdemServico');
}
...
function goToDetail(id: any) {
  router.push({ path: `/OrdemServico/detailsOrdemServico/${id}` });
}
...
const searchOrdemServico = () => {
  filterOrdemServico();
};

const filterOrdemServico = () => {
  if (!search.value) {
    filteredOrdemServico.value = OrdemServico.value;
  } else {
    const searchQuery = search.value.toLowerCase();
    filteredOrdemServico.value = OrdemServico.value.filter(item =>
